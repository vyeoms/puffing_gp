...
typedef struct {
    int        dim, n, cap;
    float    *X;               // (cap x dim) row-major
    float    *y;               // (cap,)
    float    *L;               // (cap x cap) buffer; leading n x n is lower Cholesky of K_y, row-major
    float    *alpha;           // (cap,) buffer; leading n is K_y^-1 * y
    float     raw_noise;
...
float gp_get_noise(const GP *gp);
void   gp_set_noise(GP *gp, float v);
...
void gp_predict(const GP *gp, const float *Xs,
                float *means, float *vars, int m);
...
//   kernel_grads: array of gp->kernel->n_params floats (pass NULL to skip)
